...
int InitPS2()
{
  error = ps2X.config_gamepad(A2, A4, A3, A5);   //设置PS2接口 A2号IO为clock,A4号IO为command,A3号IO为attention,A5号IO为data
  return error;
}
...
void ps2Handle() {                      //PS2 手柄 处理
  ...
  if (Mode == 0)
  {
    ...
    else
    {
      ...
      if (ps2X.Button(PSB_R1)) { //如果右侧R1按钮被按下�
        ServoPwmDutyset[2] -= 30;
        if (ServoPwmDutyset[2] < 500)
          ServoPwmDutyset[2] = 500;
        ServoSetPluseAndTime( 2, ServoPwmDutyset[2], 60 );
      }
      if (ps2X.Button(PSB_L2)) { //如果左侧L2按钮被按下�
...
//      if (ps2X.Analog(PSS_LX) > 200) //摇杆的数值大于128则执行
//      {
//        ServoPwmDutyset[4] += 30;
//        if (ServoPwmDutyset[4] > 2500)
//          ServoPwmDutyset[4] = 2500;
